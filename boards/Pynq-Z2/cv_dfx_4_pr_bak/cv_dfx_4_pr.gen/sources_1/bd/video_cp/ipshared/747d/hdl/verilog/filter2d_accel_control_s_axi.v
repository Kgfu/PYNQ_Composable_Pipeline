...
// 0x1c : reserved
// 0x20 : Data signal of shift
//        bit 7~0 - shift[7:0] (Read/Write)
//        others  - reserved
// 0x24 : reserved
// 0x40 ~
// 0x5f : Memory 'filter' (9 * 16b)
//        Word n : bit [15: 0] - filter[2n]
//                 bit [31:16] - filter[2n+1]
...
localparam
    ...
    ADDR_SHIFT_DATA_0 = 7'h20,
    ADDR_SHIFT_CTRL   = 7'h24,
    ADDR_FILTER_BASE  = 7'h40,
    ADDR_FILTER_HIGH  = 7'h5f,
    WRIDLE            = 2'd0,
    WRDATA            = 2'd1,
    WRRESP            = 2'd2,
    WRRESET           = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [0:0]                    int_filter_shift;

//------------------------Instantiation------------------
// int_filter
filter2d_accel_control_s_axi_ram #(
    .BYTES    ( 4 ),
    .DEPTH    ( 5 )
) int_filter (
    .clk0     ( ACLK ),
...
assign RVALID  = (rstate == RDDATA) & !int_filter_read;
...
//------------------------Memory logic-------------------
// filter
assign int_filter_address0 = filter_address0 >> 1;
assign int_filter_ce0      = filter_ce0;
assign int_filter_we0      = 1'b0;
assign int_filter_be0      = 1'b0;
assign int_filter_d0       = 1'b0;
assign filter_q0           = int_filter_q0 >> (int_filter_shift * 16);
assign int_filter_address1 = ar_hs? raddr[4:2] : waddr[4:2];
...
always @(posedge ACLK) begin
    if (ACLK_EN) begin
        if (filter_ce0)
            int_filter_shift <= filter_address0[0];
    end
end
...
module filter2d_accel_control_s_axi_ram
#(parameter
    BYTES  = 4,
    DEPTH  = 256,
    AWIDTH = log2(DEPTH)
